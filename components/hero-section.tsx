      ...
      <div className='relative mx-auto max-w-7xl px-4 sm:px-6 lg:px-8'>
        ...
        <div className='mx-auto max-w-4xl text-center'>
          ...
          <p className='mx-auto mt-6 max-w-2xl text-lg text-muted-foreground sm:text-xl lg:text-2xl'>
            A structured assessment for electrical engineering students. Take a
            15-minute test, get ranked results, and follow a clear roadmap to
            your specialization.
          </p>
            ...
            <div className='relative rounded-xl border border-border/60 bg-[#141414] backdrop-blur-sm overflow-hidden shadow-2xl min-w-[900px] lg:min-w-0'>
              ...
              <div className='grid grid-cols-4 gap-4 p-4 border-b border-border/60 bg-[#181818]'>
                {[
                  ...
                  <div
                    key={i}
                    className='flex items-center gap-3 p-3 rounded-lg bg-[#1c1c1c] border border-border/40'>
                ...
                <div className='p-5'>
                  ...
                  <div className='space-y-3'>
                    {[
                      ...
                      <div
                        key={track.rank}
                        className='p-2.5 rounded-lg bg-[#1c1c1c] border border-border/40'>
                    ...
                    <div className='mt-2 p-2.5 rounded-lg bg-accent/5 border border-accent/20'>
                      <p className='text-[10px] text-muted-foreground leading-relaxed'>
                        Strong interest in physical systems, energy, and
                    ...
                    {journeySteps.map((step, i) => (
                      <div
                        ...
                        <div className='flex items-center gap-2'>
                          ...
                          <span
                            className={`text-xs font-mono ${step.status === 'locked' ? 'text-muted-foreground/40' : ''}`}>
                            {step.label}
                          </span>
                        </div>
                        {step.status === 'locked' && (
                          <span className='text-[9px] text-muted-foreground/40 border border-border/30 rounded px-1'>
                            PRO
                          </span>
                        )}
                        {step.status === 'active' && (
                    ...
                    <div className='space-y-1.5'>
                      {roadmapItems.map((item, i) => (
                        <div
                          ...
                          <div
                            className={`mt-1 h-2 w-2 rounded-full flex-shrink-0 ${item.free ? 'bg-accent' : 'bg-muted-foreground/30'}`}
                          />
                          <div className='flex-1 min-w-0'>
                            <p
                              className={`text-[10px] font-mono ${item.free ? 'text-accent' : 'text-muted-foreground/40'}`}>
                              {item.day}
                            </p>
                            <p
                              className={`text-[11px] truncate ${item.free ? 'text-foreground' : 'text-muted-foreground/30'}`}>
                              {item.free ? item.title : '— Premium content —'}
                            </p>
                          </div>
                          {!item.free && (
                            <Lock className='h-3 w-3 text-muted-foreground/30 flex-shrink-0 mt-0.5' />
                          )}
                        </div>
                      ))}
                    </div>
